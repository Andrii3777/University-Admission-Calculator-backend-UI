...
async function areTablesEmpty() {
    try {
        const [{ count: rowsInSpecialityTable }] = await executeQuery(queries.checkIfEmtySpecialityTable);
        const [{ count: rowsInSpecialtyExamCoefTable }] = await executeQuery(queries.checkIfEmtySpecialtyExamCoefficientTable);
