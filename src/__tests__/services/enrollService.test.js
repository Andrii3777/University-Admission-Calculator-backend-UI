...
describe('enrollService', () => {
    ...
    describe('enroll', () => {
        ...
        test('should save scores and return eligible specialties', async () => {
            ...
            jest.spyOn(enrollService, '_getAppropriateSpecialties').mockResolvedValue(mockSpecialties);

            const result = await enrollService.enroll(studentId, scores);

            expect(validateScores).toHaveBeenCalledWith(scores);
            expect(enrollService._saveStudentScores).toHaveBeenCalledWith(studentId, scores);
            expect(enrollService._getAppropriateSpecialties).toHaveBeenCalledWith(scores);
    ...
    describe('getStudentScores', () => {
        test('should retrieve and return student scores in the correct format', async () => {
            ...
            expect(executeQuery).toHaveBeenCalledWith(queries.getStudentScores, [studentId]);
