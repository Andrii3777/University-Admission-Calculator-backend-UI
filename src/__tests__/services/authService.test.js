...
describe('AuthService', () => {
    ...
    describe('signup', () => {
        it('should create a new student and return tokens on successful signup', async () => {
            ...
            expect(executeQuery).toHaveBeenCalledWith(queries.insertStudent, [email, hashedPassword]);
        });
        ...
        it('should return an error if the email is already in use', async () => {
            ...
            expect(result).toEqual({ error: 'That email is already in use', path: 'email' });
    ...
    describe('login', () => {
        it('should return tokens if the login is successful', async () => {
            ...
            executeQuery.mockResolvedValue([{ id: studentId, password: hashedPassword }]);
            ...
            expect(tokenService.saveToken).toHaveBeenCalledWith(studentId, 'refreshToken');
        });
        ...
        it('should return an error if the email does not exist', async () => {
            ...
            expect(executeQuery).toHaveBeenCalledWith(queries.getStudentByEmail, [email]);
        });
        ...
        it('should return an error if the password is incorrect', async () => {
            ...
            expect(result).toEqual({ error: 'Password is incorrect', path: 'password' });
            expect(bcrypt.compare).toHaveBeenCalledWith(password, hashedPassword);
        });
    });
    ...
    describe('logout', () => {
        it('should return true if the refresh token was deleted successfully', async () => {
            ...
            expect(executeQuery).toHaveBeenCalledWith(queries.deleteToken, [refreshToken]);
        });
    ...
    describe('Password Validation', () => {
        ...
        it('should return an error if password does not contain an uppercase letter', () => {
            ...
            expect(result).toBe('Password must contain at least one uppercase letter');  // Validate the error for missing uppercase letter
        });

        it('should return an error if password does not contain a lowercase letter', () => {
            const result = validatePassword('PASSWORD1');
            expect(result).toBe('Password must contain at least one lowercase letter');  // Validate the error for missing lowercase letter
        });
