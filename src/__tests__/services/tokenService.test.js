...
describe('TokenService', () => {
    ...
    describe('validateAccessToken', () => {
        it('should return decoded payload if the access token is valid', () => {
            ...
            expect(jwt.verify).toHaveBeenCalledWith(accessToken, env.JWT_ACCESS_SECRET);
    ...
    describe('validateRefreshToken', () => {
        it('should return decoded payload if the refresh token is valid', () => {
            ...
            expect(jwt.verify).toHaveBeenCalledWith(refreshToken, env.JWT_REFRESH_SECRET);
            expect(result).toEqual(payload);
        });
    ...
    describe('saveToken', () => {
        it('should update the refresh token if it already exists for the student', async () => {
            ...
            expect(executeQuery).toHaveBeenCalledWith(queries.updateTokenByStudentId, [refreshToken, studentId]);
        });
        ...
        it('should insert a new refresh token if it does not exist for the student', async () => {
            ...
            expect(executeQuery).toHaveBeenCalledWith(queries.insertToken, [refreshToken, studentId]);
        });
    });
    ...
    describe('refresh', () => {
        it('should return new tokens if refresh token is valid and student exists', async () => {
            ...
            tokenService.generateTokens = jest.fn().mockReturnValue({ accessToken, refreshToken });

            const result = await tokenService.refresh(refreshToken);

            expect(executeQuery).toHaveBeenCalledWith(queries.getToken, [refreshToken]);
            expect(executeQuery).toHaveBeenCalledWith(queries.getStudentById, [studentId]);
            expect(tokenService.generateTokens).toHaveBeenCalledWith({ id: studentId, email });
            expect(result).toEqual({ tokens: { accessToken, refreshToken }, studentId });
        });
