...
class AuthService {
    ...
    async signup(email, password) {
        try {
            ...
            const newStudent = await executeQuery(queries.insertStudent, [email, hashedPassword]);

            // Generate JWT tokens
            const { accessToken, refreshToken } = tokenService.generateTokens({ id: newStudent.insertId, email });
    ...
    async login(email, password) {
        try {
            ...
            // Generate JWT tokens
            const { accessToken, refreshToken } = tokenService.generateTokens({ id: student.id, email });
